...
public class TestHRegion extends HBaseTestCase {
  ...
  public void testGet_Basic() throws IOException {
    ...
    //Add to memstore
    Put put = new Put(row1);
    put.add(fam1, col1, Bytes.toBytes("S"));
    put.add(fam1, col2, Bytes.toBytes("H"));
    put.add(fam1, col3, Bytes.toBytes("E"));
    put.add(fam1, col4, Bytes.toBytes("N"));
    put.add(fam1, col5, Bytes.toBytes("G"));
    region.put(put);
    
    put = new Put(row1);
    put.add(fam1, col1, Bytes.toBytes("S_2"));
    put.add(fam1, col2, Bytes.toBytes("H_2"));
    put.add(fam1, col3, Bytes.toBytes("E_2"));
    put.add(fam1, col4, Bytes.toBytes("N_2"));
    put.add(fam1, col5, Bytes.toBytes("G_2"));
    region.put(put);
    
    // Original test, comment out by wang sheng 2013-4-26
//    Put put = new Put(row1);
...
//    region.put(put);
    ...
    {
      System.out.println(new String (res.raw()[i].getValue()));
...
//              expected[i].getQualifier(), res.raw()[i].getQualifier()));
      ...
      //Test using a filter on a Get //No need. Commented out by huanghao
//    Get g = new Get(row1);
//    final int count = 2;
...
//    assertEquals(count, res.size());
  ...
  public void testRegionScanner_Next() throws IOException {
    ...
    //Setting up region
    String method = this.getName(); //method=null here.
    initHRegion(tableName, method, families);
...
//            System.out.println("Putting of kvsetsize=" + put.size());
  ...
  throws IOException{
    ...
    //================================================================
    // code 20130425-12:00 added by huanghao. 
    // when we test HRegion, fs cannot be caught, so we check and add fs here.    
    if (fs==null)
    {
    	fs = FileSystem.getLocal(conf);//added by huanghao
    }
    // end of code 20130425-12:00
    //================================================================

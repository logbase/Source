...
public class HRegionServer implements HRegionInterface, HBaseRPCErrorHandler,
  ...
  private HServerLoad.RegionLoad createRegionLoad(final HRegion r) {
    ...
    synchronized (r.stores) {
      ...
      for (Store store : r.stores.values()) {
        storefiles += store.getStorefilesCount(); //11111111111111111111111111111huanghao
    ...
    for (Map.Entry<String, HRegion> e : this.onlineRegions.entrySet()) {
        ...
        synchronized (r.stores) {
          ...
          for (Map.Entry<byte[], Store> ee : r.stores.entrySet()) {
            ...
            storefiles += store.getStorefilesCount();//1111111111111111111111111111111huanghao
  ...
  throws KeeperException, IOException {
    // Do checks to see if we need to compact (references or too many files)
	  //===========================
	  //add by huanghao at 20130509-20:00. to avoid root-region r use operations related to storefiles
	  boolean tmp1 = r.hasReferences();
	  boolean tmp2 = false;	  
	  if (!r.regionInfo.isRootRegion())
	  {
	      tmp2 = r.hasTooManyStoreFiles();
	  }
	  //==========================
    if (tmp1 || tmp2) {
  ...
  @Override
  @QosPriority(priority=HIGH_QOS)
  public void openRegion(HRegionInfo region) //===========top enter 2.1
  throws IOException {
  ...
  @Override
  @QosPriority(priority=HIGH_QOS)
  public void openRegions(List<HRegionInfo> regions)//top enter 2.2
  throws IOException {

...
import org.apache.hadoop.hbase.regionserver.wal.LogBaseScanner;
...
/**
 * A Store holds a column family in a Region. It's comprised of two components:
 * Its a MemIndex, a set of zero or more StoreFiles, which stretch backwards
 * over time.
 * 
 * TODO: Yuting, the primary tasks include to write the keyValue into the WAL log.
 * 
 * Differ from the original Store in HBase, in which all logging and locking is
 * handled at the HRegion level, this store has to handle all the log-file.
 * In some sense, the StoreFiles is the LogFiles  
 * 
 *  Store just provides services to manage sets of
 * StoreFiles. One of the most important of those services is compaction
 * services where files are aggregated once they pass a configurable threshold.
 * 
 * <p>
 * The only thing having to do with logs that Store needs to deal with is the
 * reconstructionLog. This is a segment of an HRegion's log that might NOT be
 * present upon startup. If the param is NULL, there's nothing to do. If the
 * param is non-NULL, we need to process the log to reconstruct a TreeMap that
 * might not have been written to disk before the process died.
 * 
 * <p>
 * It's assumed that after this constructor returns, the reconstructionLog file
 * will be deleted (by whoever has instantiated the Store).
 * 
 * <p>
 * Locking and transactions are handled at a higher level. This API should not
 * be called directly but by an HRegion manager.
 */
public class Store implements HeapSize {
  ...
  /*
   * Yuting: store the offset of each new update keyValue in the log-only store
   */
  protected final MemIndex memIndex;
  protected final Path indexPath;

  /*
   * Yuting: the essential data files in the log-only store
   */
  final protected HLog currentLog;// one store, one Hlog (store ~= Hlog) with many logfiles. These logfiles can be get by . noted by huanghao
  ...
  /*
   * WangSheng: the log cache mechanism implementation for a specific store
   */
  final protected LogStoreCache logCache;
  ...
  throws IOException {
    ...
    this.indexPath = getIndexPath(basedir, info.getEncodedName(), family.getName());
    // TODO: if index Path doesn't exist?
    this.region = region;
    this.family = family;
    this.conf = conf;
...
//    this.storefiles = sortAndClone(loadStoreFiles());     // TODO: need to remove the storefiles
    this.storefiles = null;
    
    this.memIndex = new MemIndex(this);

    this.currentLog = new HLog(this.fs, this.homedir,
        this.homedir, this.conf, null, false, null);
    
    this.logCache = new LogStoreCache();
  }
  ...
  protected long add(final KeyValue kv) {
    ...
    try {
    	...
    	//wangsheng
        //System.out.println("Store.add: In Wrong Function");
  ...
  /**
   * flush the kv to the WAL, and update the memindex as well
   * This is different to the previous add() 
   *
   * TODO: 
   * 1) when to close the log file and create a new log file?
   * how to control the granularity of each file
   * 2) add the statistic info, eg: write ops, write latency, and so on
   * @param kv
   * @return memstore size delta
   */
  protected void add(List<KeyValue> kvs) throws IOException {
    lock.readLock().lock();     //TODO: is it necessary?
    
    //wangsheng
    //for (KeyValue kv : kvs) System.out.println("Store.add kv = " + kv.toString());
    
    try {
    	WALEdit edits = new WALEdit();		// TODO: be noted, this could be very slow
    	for(KeyValue kv: kvs){
    		edits.add(kv);
//    		System.out.println("TEST : "+ new String(kv.getRow()) + " | " + new String(kv.getFamily()) + " | " + new String(kv.getQualifier()) + " | " + new String(kv.getValue()));
    	}
    	
    	// insert the key values to the log, the return offsets are the start offset of each keyvalue
    	List<LogEntryOffset> offsets = this.currentLog.append(edits, EnvironmentEdgeManager.currentTimeMillis()); 
    	
    	// update Index
    	for(int i = 0;i < kvs.size();i ++){
    	  memIndex.add(kvs.get(i).getKeyForLogBaseMemIndex(), offsets.get(i));
    	  
    	  //wangsheng
    	  //System.out.println("add into index getKeyForLog() = " + kvs.get(i).toString());
    	  
    	  //Debug.debug("insert to file num: " + offsets.get(i).getFileNum());
    	}
  ...
  /**
   * StoreOffset is composed of logFilenum and offsetInFile
   * long (logFileNum, offsetInFile) 
   * 
   * TODO: I use long as the type to make the best use of the storage
   * It may need to extends or improve this mechanism later.
   * 
   * @param logFilenum the log file number where the entry is stored
   * @param offsetInFile the offset that the entry is stored in its log
   * @return
   */
  private long generateStoreOffset(int logFilenum, int offsetInFile){
    long ret = logFilenum;
    ret = ret << 32 | offsetInFile;
    return ret;
  }
  private int getLogFileNumFromStoreOffset(long storeOffset){
    return (int)(storeOffset >> 32);
  }
  private int getLogOffsetInFile(long storeOffset){
    return (int)(storeOffset & 0xFFFFFFFF);
  }

//  public void flushAndCreateNewLog() throws IOException{
//    
//    //TODO: 
//    this.lock.writeLock().lock();
//    List<HLog> tmpFilesList = new ArrayList<HLog>(logfiles.size() + 1);
//    List<Long> tmpNumList = new ArrayList<Long>(logfilenums.size() + 1);
//    for(int i = 0;i < logfiles.size();i ++){
//      tmpFilesList.add(logfiles.get(i));
//      tmpNumList.add(logfilenums.get(i));
//    }
//    
//    try{
//      if(currentLog != null)
//        currentLog.close();
//      
//      currentLog = this.createNewLog();
//      
//      tmpFilesList.add(currentLog);
//      tmpNumList.add(new Long(currentLog.getFileNum()));
//      
//      logfiles = ImmutableList.copyOf(tmpFilesList);
//      logfilenums = ImmutableList.copyOf(tmpNumList);
//    }finally{
//      this.lock.writeLock().unlock();
//    }
//  }
//  
//  private HLog createNewLog() throws IOException{
//    return new HLog(fs, homedir, null, null, conf, false, null);
//  }
...
//      return null;
  ...
  /*
   * TODO: here, this should manage the logfiles, rather than the storefiles
   * @param cache
  ...
  private void compactLog(){
    // TODO: add the compact here, and be compatible to the previous interface
  }
...
//    }
  //===================end original==========================
	  ...
	  //new added by wangsheng
	  this.lock.readLock().lock();
	  try {		
		  //LogScanner scanner = this.getLogScannerByFile(); //get the scanner of current store
		  KeyValueScanner scanner = getScanner(new Scan(), null);
		  KeyValue ret = null;
		  KeyValue tmp;	  
		  
		  while ((tmp = scanner.next()) != null)
		  {
			  if (Bytes.compareTo(tmp.getRow(), kv.getRow()) > 0) continue;
			  if (ret != null && Bytes.compareTo(ret.getKey(), tmp.getRow()) >= 0) continue;
			  ret = tmp;
		  }		  
		  
		  scanner.close();//note this step is required.
  ...
  public KeyValueScanner getScanner(Scan scan,
    ...
    try {
      	...
      	//This is replaced by logScanner in LogBase, by wangsheng 2013-4-26
    	
    	//original
    	//return new StoreScanner(this, scan, targetCols);
    	
    	//for (byte[] b : targetCols) System.out.println(new String(b));
    	
    	getLog().rollWriter();
    	
    	//KeyValueScanner scanner = new LogBaseScanner(this.currentLog, this.memIndex, this.logCache);
    	
//    	System.out.println(scanner.next());

    	return new LogBaseScanner(this.family.getName(), scan, this.currentLog, this.memIndex, this.logCache);
  ...
  int getStorefilesCount() {
	return currentLog.outputfiles.size()+1;//return all inactive log files + the active one. wangshen.
    //return this.storefiles.size();
  }
  ...
  public boolean hasTooManyStoreFiles() {
	// currently, we do not care the number of store files. wangsheng
	return false;
  ...
  public static Path getIndexPath(final Path tabledir,
      final String encodedName, final byte[] family) {
    return new Path(tabledir, new Path("index", new Path(encodedName, new Path(
        Bytes.toString(family)))));
  }

  // TODO: modify the interface here
  public LogScanner getLogScannerByKey()throws IOException{
	  //WangSheng
  	  return this.currentLog.createLogScanner(this.memIndex, LogScanner.ScannerType.ByKey, this.logCache);
  	  //return this.currentLog.createLogScanner(this.memIndex, LogScanner.ScannerType.ByKey);
  }
  
  public LogScanner getLogScannerByFile() throws IOException{
  	return this.currentLog.createLogScanner(this.memIndex, LogScanner.ScannerType.ByFile);
  }
  
  // TODO: remove this, it's just for the test
  public HLog getLog(){

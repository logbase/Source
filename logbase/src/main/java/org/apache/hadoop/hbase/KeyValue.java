...
//    return rowCache;
      ...
      short l = getRowLength();
      byte[] row = new byte[l];
      System.arraycopy(getBuffer(), o, row, 0, l);
      return row;
  }
  ...
  public long getSerializationLength(){
    return this.length + Bytes.SIZEOF_INT;
  }
  public Key getKeyForLog(){
    // getBuffer() don't need to create the byte array
    return new Key(this.getBuffer(), this.getKeyOffset(), this.getKeyLength());
  }
  
  //wangsheng
  public Key getKeyForLogBaseMemIndex(){
	 // getBuffer() don't need to create the byte array
	  
	  byte[] key = this.getKey();
	  
	  for (int i = key.length - TIMESTAMP_TYPE_SIZE; i < key.length; ++i) key[i] = (byte)0;
	  
	  //long timestamp = Bytes.toLong(b, o + (l - TIMESTAMP_TYPE_SIZE));
	  
	  return new Key(key,0,key.length);
	  
	 //return new Key(this.getBuffer(), this.getKeyOffset(), this.getKeyLength());
  }
  
  // still need the deep copy, but don't create the object...
  public void getKeyForLog(Key reuse){
    reuse.set(this.getBuffer(), this.getKeyOffset(), this.getKeyLength());
    return;
  }
  
  public static class Key implements WritableComparable<Key>, HeapSize{
    int length = 0;
    byte[] data = null;
    
    public Key(){}; 
    
    public Key(byte[] key, int keyOffset, int keyLength){
      set(key, keyOffset, keyLength);
    }
    
    public void set(byte[] key, int keyOffset, int keyLength){
      this.length = keyLength;
      if(data == null || data.length < keyLength) data = new byte[this.length];
      System.arraycopy(key, keyOffset, this.data, 0, this.length);
    }
    ...
    public int compareTo(Key o) {
      ...
      return Bytes.compareTo(this.data, 0, this.length, o.data, 0, o.length);
    }
     ...
     */
    public short getRowLength() {
      return Bytes.toShort(this.data, getKeyOffset());
    }
     ...
     */
    public int getFamilyOffset(int rlength) {
      return getKeyOffset() + Bytes.SIZEOF_SHORT + rlength + Bytes.SIZEOF_BYTE;
    }
  ...
  //WangSheng
  public void setValue(byte[] arr, int len) {
	this.length = len;
	this.offset = 0;
	this.bytes = new byte[len];
	System.arraycopy(arr, 0, this.bytes, 0, len);
  }
}

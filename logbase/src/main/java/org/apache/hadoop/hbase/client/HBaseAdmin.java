...
//  private final HConnection connection;
  ...
  throws IOException {
    HTableDescriptor.isLegalTableName(desc.getName());
    try {
      createTableAsync(desc, splitKeys);//wrong here took too long time. wangsheng huanghao
    } catch (SocketTimeoutException ste) {
      LOG.warn("Creating " + desc.getNameAsString() + " took too long", ste);
    }
    int numRegs = splitKeys == null ? 1 : splitKeys.length + 1;
    int prevRegCount = 0;
    //System.out.println("we are here #3"); //wangsheng
    
    for (int tries = 0; tries < numRetries; ++tries) {
      // Wait for new table to come on-line
    	System.out.println("we are here #4" + " tries = "+tries + " numRetries = "+numRetries); //wangsheng
...
//      //wang sheng 2013-5-13
//      boolean tmp = true;
//      if (tmp) return;
//      // -------end test
      //wangsheng
      System.out.println("actualRegCount.get() = "+actualRegCount.get()+" numRegs "+numRegs);
      
      if (actualRegCount.get() != numRegs) {
        if (tries == numRetries - 1) {
        	
        	//System.out.println("we are here #5"); //wangsheng

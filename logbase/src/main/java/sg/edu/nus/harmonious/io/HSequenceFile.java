...
package sg.edu.nus.harmonious.io;
...
/**
 * This file is copied from SequenceFile of Hadoop-0.21.0
 * Two places are modified: 
 *  1) the sync is written after append(), moreover, I set it very large, so that it won't be written to the file;
 *  2) add one function: nextKeyAtPos(Writable, long) -> this seems useless now
 */
 ...
 */
public class HSequenceFile {
  private static final Log LOG = LogFactory.getLog(HSequenceFile.class);

  private HSequenceFile() {}                         // no public ctor
...
//  public static final int SYNC_INTERVAL = 100*SYNC_SIZE; 
  public static final int SYNC_INTERVAL = Integer.MAX_VALUE;
...
//    public synchronized void append(Writable key, Writable val, LongWritable valueBeginOffset)throws IOException {
//      append((Object) key, (Object) val);
////      if(valueBeginOffset != null)
////        valueBeginOffset.set(this.valueOffset);
//    }
...
//        LongWritable keyBeginOffset, LongWritable valueBeginOffset)throws IOException{
//      this.append(key, val);
////      if(keyBeginOffset != null) keyBeginOffset.set(this.keyOffset);
////      if(valueBeginOffset != null) valueBeginOffset.set(this.valueOffset);
//    }
...
//      this.keyOffset = out.getPos() + Bytes.SIZEOF_INT + Bytes.SIZEOF_INT;
//      this.valueOffset = this.keyOffset + keyLength;
      ...
      checkAndWriteSync();                                // sync: TODO: I really think this is useless...
    }
        
//    public long getKeyBeginOffset(){ 
//      return this.keyOffset;
//    }
//    public long getValueBeginOffset(){
...
//    }
  ...
  static class RecordCompressWriter extends Writer {
    ...
    public synchronized void append(Object key, Object val)
      ...
      checkAndWriteSync();                                // sync -> sequence changed
    }
  ...
  public static class Reader implements java.io.Closeable {
    ...
    /**
     * Yuting: Read the key at the specific position
     * Currently it only consider the plain data 
     * @param offset
     * @throws IOException
     */
    public boolean nextKeyAtPos(Writable key, long offset)throws IOException{
      if(key.getClass() != getKeyClass())
        throw new IOException("wrong key class: "+key.getClass().getName()
            +" is not "+keyClass);
      
      if(blockCompressed) throw new IOException("Didn't support block compressed data"); 
      seek(offset);
      outBuf.reset();       
      keyLength = next(outBuf); //TODO: here it may has problem 
    ...
    public void updateFileSize()throws IOException{
      updateFileSize(fs.getFileStatus(this.file).getLen());
    }
    public void updateFileSize(long size){
      this.end = end > size ? end : size;
    }
    ...
    public synchronized void seek(long position) throws IOException {
      if(position >= this.end) updateFileSize();
      in.seek(position);
    ...
    public long getDataEnd() throws IOException{
      return this.end;
    }
  ...
  // Before writing the KeyValue pair to the HDFS, it
  // first writes the buffer.getLength() and keyLength 
  public static int prefixSizeForKeyValuePair(){
  	return Bytes.SIZEOF_INT + Bytes.SIZEOF_INT;
  }
